...
/**
 * Pool sizes used by Cursor's usage-summary percentages (cents).
 *
 * Empirically verified against get-filtered-usage-events + apiPercentUsed /
 * autoPercentUsed / totalPercentUsed (Ultra, 2026-07):
 *   Other Models (API)  $500
 *   Cursor Models (Auto)$2000
 *   Total               $2500
 *
 * Note: plan.limit / includedAmountCents ($400 on Ultra) is the purchased
 * included allowance before "hit usage limit", not the % bar denominator.
 */
const PLAN_POOL_LIMITS = {
  ultra: { apiLimitCents: 50_000, autoLimitCents: 200_000 },
...
function applyPoolLimits(summary, membershipType) {
  ...
  if (summary.apiUsedPercent != null && pools.apiLimitCents != null) {
  ...
  if (summary.autoPercentUsed != null && pools.autoLimitCents != null) {
  ...
  if (pools.apiLimitCents != null && pools.autoLimitCents != null) {
    summary.totalLimitCents = pools.apiLimitCents + pools.autoLimitCents;
    if (summary.totalPercentUsed != null) {
      summary.totalUsedCents = (summary.totalPercentUsed / 100) * summary.totalLimitCents;
    }
  }
 ...
 * is unavailable. Authoritative list comes from GetCurrentPeriodUsage.
 */
 ...
 * (Auto / Composer / Grok 4.5 / Vega), not the Other Models (API) pool.
 */
function isAutoModel(model, autoBucketModels = DEFAULT_AUTO_BUCKET_MODELS) {
  ...
  // Known Cursor Models families (covers stale/partial bucket lists)
  ...
  if (normalized === 'grok-4.5' || normalized.startsWith('grok-4.5-')) return true;
