...
/**
 * Fallback first-party (formerly Auto + Composer) bucket models when Connect RPC
 * autoBucketModels is unavailable. Authoritative list comes from GetCurrentPeriodUsage.
 */
...
/**
 * True if the usage-event model draws from the first-party pool
 * (Auto / Composer / Grok 4.5 / Vega), not the API pool.
 */
function isAutoModel(model, autoBucketModels = DEFAULT_AUTO_BUCKET_MODELS) {
  ...
  // Known first-party families (covers stale/partial bucket lists)
